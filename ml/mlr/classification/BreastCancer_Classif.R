...
## Generate the task
task <- makeClassifTask(data = BreastCancer[,-1], target = "Class")
...
## Print & Plot
tibble(learner = names(bmr_res$BreastCancer), 
       acc.test.mean = map_dbl(bmr_res$BreastCancer, "acc.test.mean"), 
       timetrain.test.mean = map_dbl(bmr_res$BreastCancer, "timetrain.test.mean")) %>% 
