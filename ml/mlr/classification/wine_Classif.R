...
## Generate the task
task <- makeClassifTask(data = wine, target = "Class")

## Task Info
task
# Supervised task: wine
# Type: classif
# Target: diabetes
# Observations: 768
# Features:
...
## Print & Plot
tibble(learner = names(bmr_res$wine), 
       acc.test.mean = map_dbl(bmr_res$wine, "acc.test.mean"), 
       timetrain.test.mean = map_dbl(bmr_res$wine, "timetrain.test.mean")) %>% 
