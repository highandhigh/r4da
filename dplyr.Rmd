...
```{r, echo=FALSE}
rownames(employee) <- 1:15
employee$salary <- c(12000, 8000, 9000, 7000, 9500, 8800, 7200, 7800, 9000, 12800, 10000, 9600, 8800, 11000, 10000)
employee$title <- c("工程师", "分析师", "工程师", "工程师", "分析师", "分析师", "分析师", "工程师", "工程师", "分析师",  "工程师",  "工程师",  "分析师", "分析师", "工程师")
```
...
窗口函数在 dplyr 的介绍文档中，是不包括哪些与行处理没有关系的普通函数的，比如 nchar() 统计字符个数等函数，可以说是广义上的窗口函数。

下面重点介绍狭义的窗口函数，这和 SQL 中的分析函数是相对应的：它们处理的问题相同，只是处于在 R 和 SQL 两个不同的环境中。
